...
function AddNewAccount( iAgentID in number, iCurrId in number, 
      sAccount in varchar2, viewName in varchar2, iOptions in Integer default 0) return integer;

function  GetNewAccountId(iAgentId in number, iCurrId in number, 
                        sAccount in varchar2, viewName in varchar2) return number;
...
--function GetDelRowCount( iAccntID in integer,
...
begin
 ...
 --
function AddNewAccount( iAgentID in number, iCurrId in number, 
      sAccount in varchar2, viewName in varchar2, iOptions in Integer default 0) return integer is
begin
  ...
  VALUES (iAgentID, iCurrID, viewName, sAccount,
...
--  
function GetNewAccountId( iAgentId in number, iCurrId in number, 
                        sAccount in varchar2, viewName in varchar2) return number is
begin
  return AddNewAccount(iAgentId, iCurrId, sAccount, viewName, 2);
--  COMMIT;  
--  return GetAccountID(iAgentId$,iCurrId$,sAccnt$);
  exception
    when no_data_found then 
      return 0;   
end GetNewAccountId; 
...
--  возврат ID валюты по литералу
-- изва€но: 02-03.06.2004, 20.04.2006, 24.09.2013
...
/******************************************************************************
   ...
   OUT:         acntID$ - идентификатор счета по таблице fin$CORR_PAY_ACNT
             ...
             shortName$ - наименование счета в виде "компани€-валюта"
                 iFlag$ - 0 - ок; 1 -новый счет, -1 заблокирован, -2 закрыт, -4 крива€ валюта
...
BEGIN
  ...
  IF iAcntid$ <=0 THEN               
    iAcntID$ := GetNewAccountId(iAgentId$, iCurrId$, sAccnt$, ShortName$);
    if iAcntID$ >0 then
      iFlag$ := 1;
    else
      iFlag$ := -8;
...
FUNCTION GetExtBalanceInUSD RETURN NUMBER 
 IS
  fValue number := 0.0; -- сумма приведенных внешних
BEGIN
  for tmpVar in ( 
...
FUNCTION GetBalanceDeltaInUsd( StartDate Date, OnDate Date, fAddVal number default 0) RETURN NUMBER
 IS
  ...
  vClients  number := 0.0; -- приведенна€ сумма по клиентам
  vOwned    number;
  UnChained number := 0.0; 
  vbDelta   number := 0.0; -- итого отклонение в ё—ƒ
BEGIN
  UnChained := GetUnChainedInUSD(StartDate, OnDate);   -- неопределенные 	
  vExternal := GetExtBalanceInUSD();                   -- на внешних
  vClients  := GetCliBalanceInUsd();                   -- клиентские
  vOwned    := GetVBBalanceInUSD(OnDate);              -- собственные 
  vbDelta   := vExternal - vOwned - vClients - UnChained;
  RETURN Trunc(vbDelta + fAddVal, 2);                  -- подсчет отклонени€ баланса
END;
...
begin
  ...
  vOwned    := GetVBBalanceInUSD(OnDate);              -- собственные
  UnChained := GetUnChainedInUSD(StartDate, OnDate);   -- неопределенные 	
  vExternal := GetExtBalanceInUSD();                   -- внешние
  vClients  := GetCliBalanceInUsd();                   -- чужие 
...
/******************************************************************************
   PURPOSE:  установить пор€док сортировки, убить незакрытые сессии, установить идентификатор модул€ дл€ Ѕƒ
   INPUT  :    module_name$ - им€ программного  модул€
                   moduleID$ - строка идентификатора версии модул€       
